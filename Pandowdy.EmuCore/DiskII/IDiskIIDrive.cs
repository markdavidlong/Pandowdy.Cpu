...
public interface IDiskIIDrive : IRestartable
{
    ...
    /// <see cref="DiskIIControllerCard"/> for operations that require direct
    ...
    /// <para>
    /// <strong>Internal API:</strong> This property is intended for use by
    /// <see cref="DiskIIControllerCard"/> to access dirty flags and destination
    /// paths for status reporting.
    /// </para>
    /// <para>
    /// Returns null if no disk is inserted, or if the drive is using a provider that doesn't
    /// wrap an <see cref="InternalDiskImage"/> (e.g., direct file-based providers).
    /// </para>
    /// </remarks>
    InternalDiskImage? InternalImage { get; }
}
