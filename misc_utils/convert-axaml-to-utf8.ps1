# PowerShell script to convert all AXAML files to UTF-8 without BOM
# Run this from the solution root directory

Write-Host "Converting all .axaml files to UTF-8 without BOM..." -ForegroundColor Cyan

# Define directories to process (main Pandowdy projects only)
$directories = @(
    "Pandowdy.UI",
    "Pandowdy"
)
...
foreach ($dir in $directories) {
    if (Test-Path $dir) {
        ...
        Get-ChildItem -Path $dir -Filter "*.axaml" -Recurse | ForEach-Object {
            $fileCount++
            $file = $_
